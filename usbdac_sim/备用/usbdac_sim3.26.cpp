...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	*/
	...
	for (int i = 0; i < IN_RQUEUE_LEN; i++)
	{
		...
		inNode[i].data = (UCHAR *)malloc(inNode[i].datalen * sizeof(dacData));
	...
	for(int i = 0; i <OUT_RQUEUE_LEN;i++)
	{
		...
		outNode[i].data = (UCHAR *)malloc(outNode[i].datalen * sizeof(dacData));
	}
...
static void  testIn(void *)
{
	...
	while (1)
	{
			...
			//WaitForSingleObject(inMutex, INFINITE);//需要等待数据生产者产生数据
		//	WaitForSingleObject(hjkInOvLap[inQueIndex].hEvent, INFINITE);
		//输入节点入队
			//interOfUSB.interfaceForIn(inNode[inQueIndex], temMutex);//temMutex无用
