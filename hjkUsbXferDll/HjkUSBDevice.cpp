...
CCyBulkEndPoint  *HjkUSBDevice::getHjkUSBBulkOutEndPt(CCyUSBDevice *hjkUDve)
{
	if (hjkUDve->DeviceCount() < 1)
	{
		cout << "Have no an useful usb device." << endl;
		exit(1);
	}
	//int eptCnt = hjkUDve->EndPointCount();
	//for (int i = 1; i < eptCnt; i++)
	//{
		//CCyUSBEndPoint *ept = hjkUDve->BulkOutEndPt();
		
	//}
	return  hjkUDve->BulkOutEndPt;
